...
class DeviceSession(SQLModel, table=True):
    ...
    session_token_hash: str = Field(sa_column=Column(CHAR(64), index=True, nullable=False, unique=True))
    ...
    device_fingerprint_hash: Optional[str] = Field(default=None, sa_column=Column(CHAR(64), index=True, nullable=True))
...
class DeviceAuthToken(SQLModel, table=True):
    ...
    """
    id: Optional[int] = Field(default=None, primary_key=True)
    device_session_id: int = Field(foreign_key="devicesession.id", index=True, nullable=False,on_delete="CASCADE")
